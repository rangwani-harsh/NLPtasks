...
class NaiveBayes:
  ...
  def __init__(self):
	...
	self.posTokenFreq = {}
	self.negTokenFreq = {}
	self.numOfPositives = 0
	self.numOfNegetives = 0
  ...
  def getPositive(self,word):
	if self.posTokenFreq.has_key(word):
	  return self.posTokenFreq[word]
	return 0
  def getNeg(self,word):
	if self.negTokenFreq.has_key(word):
	  return self.negTokenFreq[word]
	return 0
  def classify(self, words):
	...
	probWordPos = priorPos
	negWordPos = priorNeg
	for x in words:
		if x not in wordsAdded:
			wordsAdded.append(x)
			probWordPos += math.log(1 + 1.0*self.getPositive(x))
			probWordPos -= math.log(self.totalPosWords + 2*len(self.tokens) + 1) 
			negWordPos += math.log(1 + 1.0*self.getNeg(x))
			negWordPos -= math.log(self.totalNegWords + 2*len(self.tokens) + 1)

	if probWordPos > negWordPos:
	  return 'pos'
	return 'neg'
  ...
  def addExample(self, klass, words):
	...
	if  not self.filterStopWords:
	  words = self.filterStopWords(words)
	if(klass == 'pos'):
	  self.numOfPositives +=1
	  for wd in words:
		if wd != '':
		  if wd not in wordsAdded:
			wordsAdded.append(wd)
			self.tokens.add(wd)
			self.totalPosWords += 1
			if self.posTokenFreq.has_key(wd):
			  self.posTokenFreq[wd] += 1
			else:
			  self.posTokenFreq[wd] = 1
	else:
	  self.numOfNegetives+=1
	  for wd in words:
		if wd != '':
		  if wd not in wordsAdded:
		  	  wordsAdded.append(wd)
			  ...
			  if self.negTokenFreq.has_key(wd):
				self.negTokenFreq[wd] += 1
			  else:
				self.negTokenFreq[wd] = 1
  ...
  def filterStopWords(self, words):
	...
	* Filters stop words found in self.stopList.
	...
	newWords = []
	for x in words:
	  if x not in self.stopList and x.strip()!='':
		newWords.append(x)
